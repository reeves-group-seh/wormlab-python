...
class Atom[T]:
    ...
    *subscribe* to it. Whenever the value changes, every registered callback is
    run, allowing UI components to update themselves in response to state
    changes without polling. This is the primitive behind cross-screen state and
    reactive `Component` updates.

    Only the value stored in an `Atom` should be treated as observable; the
    value itself must be immutable. Changes are detected with `!=`, so mutating
    a value in place (e.g. appending to a list) will not trigger subscribers.
    ...
    `Atom.subscribe` returns a callable that removes the callback when invoked.
    Call it once the subscriber no longer needs updates (for example, when a
    ...
    def __init__(self, value: T) -> None:
        ...
        :param value: The value for the `Atom` to store. This value should be
            immutable.
        """
    ...
    def subscribe(self, cb: Callable[[], None]) -> Callable[[], None]:
        ...
        :return: An unsubscribe lambda. When executed, the given callback will
            no longer be ran when the value updates.
        """
        self._subs.append(cb)
        return lambda: self._unsubscribe(cb)

    def _unsubscribe(self, cb: Callable[[], None]) -> None:
